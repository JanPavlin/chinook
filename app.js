...
// spremenljivka max prikazov na stran
var perpage = 20;
...
var artists = function(page, artist, details, callback) {
  db.all("SELECT Artist.ArtistId, Name, StarsNo " +
    ...
    "ORDER BY Name LIMIT " + perpage + " OFFSET ($page - 1) * " + perpage,    // konkatinacija nizov
  ...
  db.all("SELECT DISTINCT Genre.Name FROM Genre, Track, Album " +
    ...
    {$artist: artist}, function(error, rows) {
      if (error) {
        ...
        if (rows.length == 0)
          result += 'No genres found for this artist';
        else
          rows.forEach(function (row, i) {
            // preveris ce je zadnji !!!
            result += '<span genres="' + row.GenreId + '"><em>"' + row.Name + '"</em> ' + '</span>' + (i==rows.length-1 ? "" : " | ");
